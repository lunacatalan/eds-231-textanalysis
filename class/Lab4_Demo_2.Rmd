...
# logistic regression because predicting on continuous variable
lasso_spec <- logistic_reg(penalty = 0.01, # shrink some coefficients to 0
                           mixture = 1 # tell it its a pure lasso
                           ) %>% 
  set_mode("classification") %>% 
  set_engine("glmnet")
...
lasso_wf<- workflow() %>% 
  ...
  add_recipe(recipe) # same recipe from the beginning
...
lasso_rs <- fit_resamples(
  lasso_wf,
  resamples = incidents_folds, # folds we created
...
tune_spec <- logistic_reg(penalty = tune(), # shrink some coefficients to 0
                           mixture = 1 # tell it its a pure lasso
                           ) %>% 
  set_mode("classification") %>% 
  set_engine("glmnet")
...
lambda_grid <- grid_regular(
  penalty(), 
  levels = 30 # how many different values we want to try
...
```{r plot_metrics}
autoplot(tunes_rs) +
  labs(title = "lasso performance across reg. penalties")

```

```{r penalty-show-best}
show_best(tunes_rs, metric = "roc_auc")
show_best(tunes_rs, metric = "accuracy")
...
- assigned coefficients to each of the features (which means a coefficeint for each token/ word in the model)

```{r}
# fit the model with worflow and training data
fitted_lasso <- fit(final_lasso, incidents_train)
fitted_lasso
```

First let's look at the words associated with an accident being non-fatal.
- the coefficients are showing the expected occurance
...
fitted_lasso %>% 
  extract_fit_parsnip() %>% 
  tidy() %>% 
  arrange(estimate) # arange by lowest values
```
