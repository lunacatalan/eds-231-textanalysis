...
```{r message=FALSE, warning=FALSE}
options(timeout = 100)
glove6b <- read_table('data/glove.6B.100d.txt', col_names = FALSE)
...
protection %>% 
  ...
  coord_flip() +
  theme_minimal() +
  labs(x = NULL, title = "Which word vectors are most similar to protection, management, or agricultural in GloVe?")

```

The plots for these word distributions are much different than the ones that I ran above. The words have much more synonym similarity whereas when I plotted the graphs above there were a lot more insights into the contextual similarities between words. The implications of using these embeddings are that its hard to find the specific associations within a single topic. Additionally, the degree of association betewen these words is much lower than the first plot. 
...
For the word vector synonyms, the themes are more related. For the `groundwater_management` combination, the GloVe embeddings make more sense than the one that I did before. There are much more words related to types of management (eg. catchment) and what you would want to manage (eg. stormwater, wastewater). For `groundwater_community` combination the GloVe embeddings were not as intuitive as mine were. For `groundwater_no_community` the results were very similar, and were talking about contaminants and pollution. 


In general, the GloVe word embeddings were more general, but the word_vector equations gave some interesting insights. 
